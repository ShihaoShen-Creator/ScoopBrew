...
Nothing under `Library/Homebrew` is modified. This directory is self-contained
and the upstream clone can be re-synced at any time.
...
```cmd
windows\bin\brew.cmd install git
windows\bin\brew.cmd list
```

Or put `windows\bin` and Scoop's shim directory on `PATH` once:

```powershell
[Environment]::SetEnvironmentVariable('Path', $env:Path + ';D:\ScoopBrew\windows\bin;' + (Join-Path $env:USERPROFILE 'scoop\shims'), 'User')
```
...
```
windows/
  bin/brew.ps1      entry point: flag parsing, command dispatch
  bin/brew.cmd      cmd.exe wrapper so `brew` resolves without a shell hook
  lib/ScoopBrew.psm1  Scoop discovery, manifest index, version compare, output
...
```powershell
Invoke-Pester -Script 'windows/test'
```
